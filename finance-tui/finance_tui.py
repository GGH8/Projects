...
from textual.widgets import Button, Collapsible, DataTable, Footer, Header, Input, Select, Static
...
class FinanceApiClient:
    ...
    def get_months(self) -> list[str]:
        response = self._request("GET", "/transactions/months")
        data = response.json()

        if not isinstance(data, list):
            raise FinanceApiError("GET /transactions/months returned unexpected data")

        return [str(month) for month in data]
...
class FinanceApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical(id="topbar"):
            ...
            with Horizontal(id="filter-row"):
                yield Select([("All months", "")], id="filter-month", prompt="All months")
    ...
    def on_mount(self) -> None:
        ...
        self.load_month_filter_options()
        self.refresh_all()
        self.focus_table("table-income")

    def load_month_filter_options(self) -> None:
        select = self.query_one("#filter-month", Select)
        ...
        options: list[tuple[str, str]] = [("All months", "")]
        options.extend((month, month) for month in months)
    ...
    def get_filters(self) -> tuple[str, str]:
        month_value = self.query_one("#filter-month", Select).value
        month_filter = "" if month_value == Select.BLANK else str(month_value)
    ...
    def action_focus_month_filter(self) -> None:
        self.query_one("#filter-month", Select).focus()
    ...
    def set_group_title(self, group_id: str, base_title: str, rows: list[dict[str, Any]]) -> None:
        done_rows = [
            row for row in rows
            if str(row.get("status", "done")).lower() == "done"
        ]
    ...
    def get_selected_transaction_label(self) -> str:
        ...
        selected = next(
            (row for row in self.cached_rows if int(row["id"]) == transaction_id),
            None,
        )
        if selected is None:
            return "No transaction selected"
        ...
        return (
            ...
            f"{float(selected['amount']):.2f} RON | "
            f"{selected.get('status', 'done')} | {selected['description']}"
        )
    ...
    def action_edit_selected(self) -> None:
        ...
        row = next(
            (item for item in self.cached_rows if int(item["id"]) == transaction_id),
            None,
        )
        if row is None:
    ...
    def save_transaction_form(self) -> None:
        ...
        try:
            raw_amount = float(
                self.query_one("#tx-amount", Input).value.replace(",", ".").strip()
            )
        except ValueError:
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        if button_id == "apply-filters":
            ...
            self.query_one("#filter-month", Select).value = ""
